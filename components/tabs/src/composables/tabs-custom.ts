...
export const useTabsCustomStyle = (props: TabsProps) => {
  ...
  // 解析颜色
  const [bgColorClass, bgColorStyle] = useComponentColor(toRef(props, "bgColor"), "bg");
  const [barColorClass, barColorStyle] = useComponentColor(toRef(props, "barColor"), "bg");
  ...
  const barStyle = computed<CSSProperties>(() => {
    ...
    // 设置滑块的高度
    if (props.barHeight) style.height = formatDomSizeValue(props.barHeight);
    // 设置滑块的宽度
